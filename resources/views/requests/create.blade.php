<x-app-layout>
    @section('title', 'New Marketing Request')

    <x-slot name="header">
        <h1 class="text-xl font-semibold text-gray-800">New Marketing Request</h1>
        <p class="text-sm text-gray-500 mt-0.5">Fill in the form below and submit to Marcom for review.</p>
    </x-slot>

    <div class="py-4">
        <form method="POST" action="{{ route('requests.store') }}" enctype="multipart/form-data" x-data="requestForm()">
            @csrf
            ...
            <div class="bg-white rounded-xl border border-gray-100 shadow-sm mb-5">
                ...
                <div class="p-6 grid grid-cols-1 sm:grid-cols-2 gap-5">
                    ...
                    <div>
                        ...
                        <input type="text" name="pic_name" value="{{ old('pic_name', auth()->user()->name) }}"
                    ...
                    <div>
                        <label class="block text-xs font-medium text-gray-600 mb-1.5">Date Request <span
                                class="text-red-500">*</span></label>
                        <input type="date" name="date_request" value="{{ old('date_request', date('Y-m-d')) }}"
                    ...
                    <div class="flex flex-wrap gap-6">
                        <label class="flex items-center gap-2 cursor-pointer">
                            <input type="radio" name="campaign_type" value="lokal" x-model="campaignType"
                                {{ old('campaign_type') === 'lokal' ? 'checked' : '' }}
                        ...
                        <label class="flex items-center gap-2 cursor-pointer">
                            <input type="radio" name="campaign_type" value="group" x-model="campaignType"
                                {{ old('campaign_type') === 'group' ? 'checked' : '' }}
                                class="w-4 h-4 text-blue-600 border-gray-300 focus:ring-blue-300">
                            ...
                            <div class="flex gap-4" x-show="forAftersales" x-cloak x-transition>
                                <label class="flex items-center gap-2 cursor-pointer">
                                    <input type="radio" name="aftersales_vehicle_type" value="passenger_car"
                                        {{ old('aftersales_vehicle_type') === 'passenger_car' ? 'checked' : '' }}
                                        class="w-4 h-4 text-blue-600 border-gray-300 focus:ring-blue-300">
                                    <span class="text-sm text-gray-600">Passenger Car</span>
                                </label>
                                <label class="flex items-center gap-2 cursor-pointer">
                                    <input type="radio" name="aftersales_vehicle_type" value="commercial_vehicle"
                                        {{ old('aftersales_vehicle_type') === 'commercial_vehicle' ? 'checked' : '' }}
                                        class="w-4 h-4 text-blue-600 border-gray-300 focus:ring-blue-300">
                                    <span class="text-sm text-gray-600">Commercial Vehicle</span>
                                </label>
                            </div>
                        ...
                        <div class="flex flex-wrap items-center gap-4">
                            ...
                            <div x-show="forOthers" x-cloak x-transition class="flex-1 min-w-48">
                                <input type="text" name="others_description"
                                    value="{{ old('others_description') }}" placeholder="Please specify..."
                ...
                <div class="p-6 space-y-5">
                    ...
                    <div>
                        ...
                        <textarea name="detail_concept" rows="3" placeholder="Describe the concept in detail..."
                            class="w-full px-3 py-2 text-sm border border-gray-200 rounded-lg focus:ring-2 focus:ring-blue-300 focus:border-blue-400 outline-none transition resize-none @error('detail_concept') border-red-400 @enderror">{{ old('detail_concept') }}</textarea>
                    ...
                    <div>
                        ...
                        <textarea name="further_information" rows="3" placeholder="Any additional information..."
                            class="w-full px-3 py-2 text-sm border border-gray-200 rounded-lg focus:ring-2 focus:ring-blue-300 focus:border-blue-400 outline-none transition resize-none">{{ old('further_information') }}</textarea>
                    </div>
                        ...
                        <div class="border-2 border-dashed border-gray-200 rounded-lg p-6 text-center hover:border-blue-300 transition-colors"
                            ...
                            <label class="cursor-pointer">
                                <span class="text-sm text-blue-600 font-medium hover:underline"
                                    x-text="fileName || 'Click to upload or drag & drop'">Click to upload or drag &
                                    drop</span>
                        ...
                        @foreach ($mediaOptions as $value => $label)
                            <label
                                ...
                                <input type="checkbox" name="output_media[]" value="{{ $value }}"
                                    {{ in_array($value, $oldMedia) ? 'checked' : '' }}
                                    class="w-4 h-4 text-blue-600 border-gray-300 rounded focus:ring-blue-300">
    ...
    <script>
        function requestForm() {
            return {
                forSales: {{ old('for_sales') ? 'true' : 'false' }},
                forAftersales: {{ old('for_aftersales') ? 'true' : 'false' }},
                forOthers: {{ old('for_others') ? 'true' : 'false' }},
